...
namespace ORDA
{
	public class ORDA_main : Part
	{
		...
		Vessel flyVessel = null;
		int flyVesselParts = 0;
		...
		private void windowGUI (int windowID)
		{
			...
			if (currentPage == PageType.PAGE_TARGET) {
				...
				if(highlightedVesselDockingPort != vesselDockingPort && highlightedVesselDockingPort != null) {
					highlightedVesselDockingPort.highlight(ColorHighlightNone);
				}
				if(highlightedTargetDockingPort != targetDockingPort && highlightedTargetDockingPort != null) {
					highlightedTargetDockingPort.highlight(ColorHighlightNone);
				}

				// highlight selected ports
				if(vesselDockingPort != null) {
					vesselDockingPort.highlight(Color.green);
				}
				if(targetDockingPort != null) {
					targetDockingPort.highlight(Color.green);
				}
		...
		private void windowRendezvousGUI (GUIStyle style, GUIStyle activeStyle)
		{
			...
			// att
				...
				if (GUILayout.Button ("RPP", (gncAttMode == GNC.AttMode.RPP) ? (activeStyle) : (style))) {
					gnc.requestAttMode (GNC.AttMode.RPP);
				}
				if (GUILayout.Button ("RPN", (gncAttMode == GNC.AttMode.RPN) ? (activeStyle) : (style))) {
					gnc.requestAttMode (GNC.AttMode.RPN);
				}
				if (GUILayout.Button ("RVP", (gncAttMode == GNC.AttMode.RVP) ? (activeStyle) : (style))) {
					gnc.requestAttMode (GNC.AttMode.RVP);
				}
				if (GUILayout.Button ("RVN", (gncAttMode == GNC.AttMode.RVN) ? (activeStyle) : (style))) {
					gnc.requestAttMode (GNC.AttMode.RVN);
				}
				GUILayout.EndHorizontal ();
				GUILayout.BeginHorizontal ();
				if (GUILayout.Button ("VP", (gncAttMode == GNC.AttMode.VP) ? (activeStyle) : (style))) {
					gnc.requestAttMode (GNC.AttMode.VP);
				}
				if (GUILayout.Button ("VN", (gncAttMode == GNC.AttMode.VN) ? (activeStyle) : (style))) {
					gnc.requestAttMode (GNC.AttMode.VN);
				}
				if (GUILayout.Button ("RP", (gncAttMode == GNC.AttMode.RP) ? (activeStyle) : (style))) {
					gnc.requestAttMode (GNC.AttMode.RP);
				}
				if (GUILayout.Button ("RN", (gncAttMode == GNC.AttMode.RN) ? (activeStyle) : (style))) {
					gnc.requestAttMode (GNC.AttMode.RN);
				}
				if (GUILayout.Button ("NP", (gncAttMode == GNC.AttMode.NP) ? (activeStyle) : (style))) {
					gnc.requestAttMode (GNC.AttMode.NP);
				}
				if (GUILayout.Button ("NN", (gncAttMode == GNC.AttMode.NN) ? (activeStyle) : (style))) {
			...
			relVectorsToggle = GUILayout.Toggle (relVectorsToggle, "Rel Vel & Pos", GUILayout.ExpandWidth (true));
			referenceToggle = GUILayout.Toggle (referenceToggle, "Ref. Frame", GUILayout.ExpandWidth (true));
			GUILayout.EndHorizontal ();
		...
		protected override void onPartFixedUpdate ()
		{
			...
			if (flyVessel == null || flyVessel != this.vessel || flyVesselParts != this.vessel.parts.Count) {
				...
				flyVessel = this.vessel;
				flyVesselParts = this.vessel.parts.Count;
			}
			...
			// use internal propulsion if this is not the active vessel
			if ((vessel != FlightGlobals.ActiveVessel) || !isControllable) {
		...
		{
			return "[" + this.vessel.vesselName + ";" + vessel.parts.Count + ";" + this.inverseStage + "]";
		}
	}
}